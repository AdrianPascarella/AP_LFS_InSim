...
class LFSConfigManager:
    ...
    def validate_path(self) -> bool:
        ...
        if not os.path.exists(self.cfg_path):
            raise InSimConfigurationError(
                f"Archivo crítico no encontrado: {self.cfg_path}. Verifica la ruta de LFS."
            )
        return True
    ...
    def read_config(self) -> Dict[str, str]:
        ...
        try:
            ...
            raise InSimConfigurationError(
                f"Error al leer la configuración de LFS ({self.cfg_path}): {e}"
            ) from e
    ...
    def update_config(self, updates: Dict[str, str], backup: bool = True) -> bool:
        ...
        if backup:
            ...
            try:
                ...
                raise InSimConfigurationError(
                    f"Fallo al crear el backup de seguridad de cfg.txt: {e}"
                ) from e
        ...
        try:
            ...
            raise InSimConfigurationError(
                f"Error al escribir cambios en cfg.txt de LFS: {e}"
            ) from e
