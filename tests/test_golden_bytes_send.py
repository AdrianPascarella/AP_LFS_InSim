...
class TestGoldenControl:
    ...
    def test_small_ssp(self):
        assert (
            encode(ISP_SMALL(ReqI=2, SubT=SMALL.SSP, UVal=100))
    ...
    def test_small_uval_grande(self):
        ...
        assert (
            encode(ISP_SMALL(SubT=SMALL.SSG, UVal=0xDEADBEEF))
    ...
    def test_mod(self):
        assert (
            encode(ISP_MOD(Bits16=0, RR=60, Width=1920, Height=1080))
            == b"\x05\x0f\x00\x00\x00\x00\x00\x00<\x00\x00\x00\x80\x07\x00\x008\x04\x00\x00"
        )

    def test_scc(self):
        assert (
            encode(ISP_SCC(ViewPLID=3, InGameCam=VIEW.CUSTOM))
    ...
    def test_cpp(self):
        ...
        pkt = ISP_CPP(
            Pos=Vec(X=65536, Y=-131072, Z=1024),
            H=16384,
            P=0,
            R=0,
            ViewPLID=255,
            InGameCam=VIEW.CUSTOM,
            FOV=90.0,
            Time=1000,
            Flags=8,
        )
    ...
    def test_sch(self):
        assert (
            encode(ISP_SCH(CharB=ord("A"), Flags=1)) == b"\x02\x06\x00\x00A\x01\x00\x00"
        )

    def test_ttc_sel(self):
        assert (
            encode(ISP_TTC(ReqI=1, SubT=TTC.SEL, UCID=4))
            == b"\x02=\x01\x01\x04\x00\x00\x00"
        )

    def test_oco(self):
        assert (
            encode(ISP_OCO(OCOAction=5, Index=240, Identifier=10, Data=3))
            == b"\x02<\x00\x00\x05\xf0\n\x03"
        )

    def test_bfn_clear(self):
        assert (
            encode(ISP_BFN(SubT=BFN.CLEAR, UCID=255))
            == b"\x02*\x00\x01\xff\x00\x00\x00"
        )
...
class TestGoldenMensajes:
    def test_mst_hola(self):
        ...
        assert encode(ISP_MST(Msg="hola")) == b"\x11\r\x00\x00" + b"hola" + b"\x00" * 60
    ...
    def test_mst_larga_se_trunca(self):
        ...
        assert encode(ISP_MST(Msg="x" * 70)) == b"\x11\r\x00\x00" + b"x" * 63 + b"\x00"
    ...
    def test_mst_de_64_justos_pierde_un_char_por_el_null(self):
        ...
        assert encode(ISP_MST(Msg="x" * 64)) == b"\x11\r\x00\x00" + b"x" * 63 + b"\x00"
    ...
    def test_mtc_hola(self):
        ...
        assert (
            encode(ISP_MTC(UCID=1, Text="hola"))
    ...
    def test_mtc_larga_se_trunca_a_128(self):
        ...
        assert (
            encode(ISP_MTC(UCID=1, Text="y" * 130))
            == b'"\x0e\x00\x00\x01\x00\x00\x00' + b"y" * 127 + b"\x00"
        )

    def test_msx(self):
        assert (
            encode(ISP_MSX(Msg="mensaje extendido"))
            == b"\x19'\x00\x00" + b"mensaje extendido" + b"\x00" * 79
        )

    def test_msl(self):
        assert (
            encode(ISP_MSL(Sound=SND.SYSMESSAGE, Msg="local"))
...
class TestGoldenCarrera:
    def test_plc(self):
        assert (
            encode(ISP_PLC(UCID=5, Cars=0x00000003))
    ...
    def test_jrr_con_objectinfo(self):
        ...
        pkt = ISP_JRR(
            ...
            JRRAction=JRR.RESET,
            StartPos=ObjectInfo(
                X=100, Y=-200, Zbyte=10, Flags=0x80, Index=0, Heading=128
            ),
        )
    ...
    def test_mal_dos_skins(self):
        ...
        assert (
            encode(ISP_MAL(NumM=2, SkinID=[0x00112233, 0xFFEEDDCC]))
    ...
    def test_axm_dos_objetos(self):
        pkt = ISP_AXM(
            ...
            Info=[
                ...
                ObjectInfo(
                    X=-32768, Y=32767, Zbyte=255, Flags=0x80, Index=192, Heading=255
                ),
            ],
        )
        assert encode(pkt) == (
    ...
    def test_aic_un_input(self):
        pkt = ISP_AIC(
            PLID=7, Inputs=[AIInputVal(Input=CS.THROTTLE, Time=50, Value=32768)]
        )
    ...
    def test_aic_25_inputs_se_truncan_a_20(self):
        ...
        pkt = ISP_AIC(
            PLID=1,
            Inputs=[AIInputVal(Input=i % 12, Time=i, Value=i) for i in range(25)],
        )
        assert encode(pkt) == (
...
class TestGoldenPantalla:
    ...
    def test_btn_texto_vacio(self):
        ...
        assert (
            encode(ISP_BTN(ReqI=1, ClickID=1, L=1, T=1, W=1, H=1))
    ...
    def test_ssh(self):
        assert (
            encode(ISP_SSH(ReqI=1, Name="captura"))
...
class TestGoldenSecuenciasFijas:
    ...
    def test_reo_con_dos_plids(self):
        ...
        assert encode(ISP_REO(ReqI=1, NumP=2, PLID=(9, 5))) == bytes(
            [13, 36, 1, 2, 9, 5]
        ) + bytes(46)
    ...
    def test_hcp_un_coche(self):
        ...
        assert encode(ISP_HCP(Info=[CarHCP(30, 30)])) == bytes(
            [17, 56, 0, 0, 30, 30]
        ) + bytes(62)
    ...
    def test_ipb_con_un_ban(self):
        ...
        assert encode(ISP_IPB(NumB=1, BanIPs=[(192, 168, 1, 1)])) == bytes(
            [3, 67, 0, 1, 0, 0, 0, 0, 192, 168, 1, 1]
        )
