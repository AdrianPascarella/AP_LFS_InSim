...
from lfs_insim.packets import (
    ISP_BTC, ISP_BTT, ISP_CNL, ISP_CON, ISP_HLV, ISP_MCI, ISP_MSO, ISP_NCN,
    ISP_NLP, ISP_NPL, ISP_OBH, ISP_SMALL, ISP_STA, ISP_TINY, ISP_VER,
)
...
class TestGoldenDecodeConexion:
    ...
    def test_sta(self):
        data = (
            ...
            + bytes([3, 1,                    # InGameCam, ViewPLID
                     2, 3, 0,                 # NumP, NumConns, NumFinished
                     1, 0, 5,                 # RaceInProg, QualMins, RaceLaps
                     0, 1])                   # Sp2, ServerStatus
...
class TestGoldenDecodeJugadores:
    ...
    def test_mci_dos_coches(self):
        ...
        car1 = struct.pack('<HHBBBBiiiHHHh',
                           250, 3, 1, 1, 64, 0,
                           655360, 1310720, 65536,     # X=10m, Y=20m, Z=1m (65536 = 1 m)
                           3000, 16384, 16000, 50)
        car2 = struct.pack('<HHBBBBiiiHHHh',
                           251, 3, 2, 2, 32, 0,
                           -655360, -1310720, 131072,
                           2500, 49152, 48000, -50)
        ...
        assert (c1.Speed, c1.Direction, c1.Heading, c1.AngVel) == (3000, 16384, 16000, 50)
...
class TestGoldenDecodeContactos:
    ...
    _CONTACTO_A = struct.pack('<BBBbBBBBBBbbhh',
                              1, 1, 0, -10, 0xF0, 0x0F, 0x30, 40,
                              100, 110, -5, 3, 1600, -1600)
    _CONTACTO_B = struct.pack('<BBBbBBBBBBbbhh',
                              2, 0, 0, 5, 0x0F, 0xF0, 0x20, 38,
                              105, 108, 4, -2, 1650, -1580)
    ...
    def test_con(self):
        data = (
            ...
            + self._CONTACTO_A + self._CONTACTO_B
        )
        assert len(data) == 44
        pkt = decode_packet(data)
