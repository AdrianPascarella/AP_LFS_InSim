...
4. Lifecycle hooks: `on_connect()`, `on_tick()` (every `tick_interval` seconds, default 0.1; min 0.01, no catch-up after stalls, not a precision timer), `on_disconnect()`, `on_reconnect()`. The main loop's internal poll stays at ≤100 ms regardless, so a slow tick never delays connection-loss detection or fail-fast errors. `INSIM_CONFIG["interval"]` (default 10 ms) controls how often LFS sends NLP/MCI, not `on_tick`; high-frequency work belongs in packet handlers (MCI/OutSim), not in `on_tick`.
