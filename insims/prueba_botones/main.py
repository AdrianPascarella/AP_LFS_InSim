...
class PruebaBotones(InSimApp):
    ...
    def on_connect(self):
        self.cmds = (
            ...
            .add_cmd(
                "colores",
                "8 botones con los colores estándar de LFS",
                None,
                self._cmd_colores,
            )
            .add_cmd(
                "estilos", "Botones LIGHT, DARK y sin estilo", None, self._cmd_estilos
            )
            .add_cmd(
                "alineacion",
                "Alineación de texto: izquierda, centro, derecha",
                None,
                self._cmd_alineacion,
            )
            .add_cmd("click", "Botón clickable: recibe IS_BTC", None, self._cmd_click)
            .add_cmd(
                "typein",
                "Botón de entrada de texto: recibe IS_BTT",
                None,
                self._cmd_typein,
            )
            .add_cmd(
                "typein_init",
                "TypeIn con texto inicial pre-rellenado",
                None,
                self._cmd_typein_init,
            )
            .add_cmd(
                "caption",
                "Botón con caption personalizado en el diálogo",
                None,
                self._cmd_caption,
            )
            .add_cmd(
                "always_on",
                "Botón visible en todas las pantallas (INST_ALWAYS_ON)",
                None,
                self._cmd_always_on,
            )
            .add_cmd(
                "update",
                "Crea un botón y luego actualiza solo su texto (W=0 H=0)",
                None,
                self._cmd_update,
            )
            .add_cmd(
                "del", "Borra un botón por su ClickID", (("id", int),), self._cmd_del
            )
            .add_cmd(
                "del_range",
                "Borra un rango de botones [from..to]",
                (("desde", int), ("hasta", int)),
                self._cmd_del_range,
            )
            .add_cmd(
                "clear", "Borra todos los botones de este InSim", None, self._cmd_clear
            )
            .submit()
        )
    ...
    def _cmd_colores(self):
        ...
        colores = [
            ISB_STYLE.LIGHT_GREY,
            ISB_STYLE.TITLE,
            ISB_STYLE.UNSELECTED,
            ISB_STYLE.SELECTED,
            ISB_STYLE.OK,
            ISB_STYLE.CANCEL,
            ISB_STYLE.TEXT_STRING,
            ISB_STYLE.UNAVAILABLE,
        ]
    ...
    def _cmd_alineacion(self):
        ...
        self.send_ISP_BTN(
            ...
            UCID=0,
            ClickID=CID_RIGHT,
            BStyle=ISB_STYLE.DARK | ISB_STYLE.RIGHT,
            L=120,
            T=10,
            W=50,
            H=10,
    ...
    def _cmd_click(self):
        ...
        self.send_ISP_MSL(
            Msg=f"{c.YELLOW}Boton clickable activo. Haz clic (LMB/RMB + CTRL/SHIFT)"
        )
    ...
    def on_ISP_BTC(self, packet: ISP_BTC):
        ...
        if packet.CFlags & ISB_CLICK.RMB:
            flags.append("RMB")
        if packet.CFlags & ISB_CLICK.CTRL:
            flags.append("CTRL")
        if packet.CFlags & ISB_CLICK.SHIFT:
            flags.append("SHIFT")
        modificadores = "+".join(flags) if flags else "sin modificadores"
        self.send_ISP_MSL(
            Msg=f"{c.GREEN}BTC: ClickID={packet.ClickID} UCID={packet.UCID} | {modificadores}"
        )
    ...
    def _cmd_typein(self):
        ...
        self.send_ISP_MSL(
            Msg=f"{c.YELLOW}TypeIn activo (max 20 chars). Haz clic y escribe."
        )
    ...
    def _cmd_typein_init(self):
        ...
        self.send_ISP_BTN(
            ...
            BStyle=ISB_STYLE.DARK | ISB_STYLE.CLICK,
            TypeIn=TYPEIN_FLAGS.INIT_WITH_TEXT
            | 20,  # pre-rellena el diálogo con el texto del botón
            L=60,
            T=20,
            W=80,
            H=12,
    ...
    def on_ISP_BTT(self, packet: ISP_BTT):
        self.send_ISP_MSL(
            Msg=f'{c.GREEN}BTT: ClickID={packet.ClickID} UCID={packet.UCID} | Texto: "{packet.Text}"'
        )
    ...
    def _cmd_caption(self):
        ...
        self.send_ISP_MSL(
            Msg=f"{c.YELLOW}Caption personalizado. Haz clic para ver el dialogo."
        )
    ...
    def _cmd_always_on(self):
        ...
        self.send_ISP_MSL(
            Msg=f"{c.YELLOW}Boton ALWAYS_ON activo (esquina sup. izq.). Abre el garage para comprobarlo."
        )
    ...
    def on_ISP_BFN(self, packet: ISP_BFN):
        if packet.SubT == BFN.REQUEST and self._active_screen:
            self.send_ISP_MSL(
                Msg=f"{c.YELLOW}BFN_REQUEST recibido. Re-enviando pantalla '{self._active_screen}'..."
            )
            ...
            self._active_screen = None
            self.send_ISP_MSL(
                Msg=f"{c.YELLOW}BFN_USER_CLEAR: el usuario limpio los botones."
            )
