...
class UsersManagement(InSimApp):
    ...
    def on_ISP_NCN(self, packet: ISP_NCN):
        """Nueva conexión."""
        user = User(packet.UName, packet.UCID, packet.PName, packet.Admin, packet.Flags)
    ...
    def on_ISP_CNL(self, packet: ISP_CNL):
        ...
        plids_a_borrar = [
            plid for plid, ai in self.ais.items() if ai.player.ucid == packet.UCID
        ]
    ...
    def on_ISP_NPL(self, packet: ISP_NPL):
        ...
        if ai.player.ucid in self.users:
            ...
            self.logger.warning(
                f"IA {packet.PName} añadida, pero su UCID dueño ({ai.player.ucid}) no está en self.users!"
            )
    ...
    def on_ISP_PLL(self, packet: ISP_PLL):
        ...
        if plid in self.players:
            ...
            self.logger.info(
                f"Salió de pista el jugador {self.users[player.ucid].player_name} y su plid volvió a None"
            )
            return
        if plid in self.ais:
            ai = self.ais.pop(plid)
            ...
            self.logger.info(
                f"Salió de pista la ai {ai.ai_name} que pertenecía al jugador {self.users[ai.player.ucid].player_name}"
            )
            return
    ...
    def user_autorized_cmd(
        self, white_list: set, cmd_base: str, cmd: str, ucid: int
    ) -> bool:
        ...
        if ucid == 0:
            return True  # El servidor siempre tiene permiso
    ...
    def on_ISP_MSO(self, packet: ISP_MSO):
        ...
        if self.user_autorized_cmd(
            self.cmds_white_list, self.cmd_base, cmd, packet.UCID
        ):
    ...
    def _show_user_info(self, searched_name: str):
        ...
        user = next(
            (u for u in self.users.values() if u.user_name == searched_name), None
        )
        ...
        if not user.plid:
            ...
            player_info = " | ".join(
                (
                    f"Plate: {player.plate}",
                    f"CarName: {player.car_name}",
                    f"SkinName: {player.skin_name}",
                )
            )
        ...
        if not user.plids_ais_actives:
            ...
            ais_info = " | ".join(
                f"PLID: {ai.player.plid}, AIName: {ai.ai_name}"
        ...
        for ai in self.ais.values():
            ...
            if telemetry:
                self.send_ISP_MSL(
                    Msg=" | ".join(
                        (
                            f"PLID: {ai.player.plid}",
                            f"DueñoUCID: {ai.player.ucid}",
                            f"AIName: {ai.ai_name}",
                            f"Car: {ai.player.car_name}",
                            f"Coordinates: ({telemetry.coordinates.x_m}, {telemetry.coordinates.y_m}, {telemetry.coordinates.z_m})",
                            f"SpeedKmh: {telemetry.speed.speed_kmh:.2f}",
                        )
                    )
                )
            else:
    ...
    def _remove_user_to_cmds_white_list(self, user_name: str):
        if user_name not in self.cmds_white_list:
            self.send_ISP_MSL(
                Msg=f'El usuario "{user_name}" no se encuentra en "cmds_white_list"'
            )
            return
