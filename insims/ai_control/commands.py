...
class _CommandsMixin(_MixinBase):
    def _init_commands(self):
        ...
        (
            self.cmds_aic.add_cmd(
                name="add",
                description="Añade una IA",
                args=None,
                funct=self._cmd_add,
            ...
            .add_cmd(
                name="my-ais",
                description="Lista de IAs activas",
                args=None,
                funct=self._show_my_ais_list,
                is_mso_required=True,
            )
            .add_cmd(
                name="speed-kmh",
                description="Velocidad constante",
                args=(("plid", int), ("speed", float)),
                funct=self._cmd_speed_kmh,
                is_mso_required=True,
            )
            .add_cmd(
                name="stop",
                description="Detiene la IA",
                args=(("plid", int),),
                funct=self._cmd_stop,
                is_mso_required=True,
            )
            .add_cmd(
                name="direction-point",
                description="Va a coordenadas X Y",
                args=(("plid", int), ("x", int), ("y", int)),
                funct=self._cmd_direction_point,
                is_mso_required=True,
            )
            .add_cmd(
                name="follow",
                description="Sigue a un PLID",
                args=(("plid_ai", int), ("plid_target", int)),
                funct=self._cmd_follow,
                is_mso_required=True,
            )
            .add_cmd(
                name="run-away",
                description="Huye de un PLID",
                args=(("plid_ai", int), ("plid_target", int)),
                funct=self._cmd_run_away,
                is_mso_required=True,
            )
            .add_cmd(
                name="adaptive",
                description="Configura velocidad adaptativa",
                args=(
                    ("plid", int),
                    ("min_speed", float),
                    ("max_speed", float),
                ...
                funct=self._cmd_adaptive,
                is_mso_required=True,
            )
            .add_cmd(
                name="spec",
                description="Pone la IA en spectate",
                args=(("plid", int),),
                funct=self._cmd_spec,
            )
            .submit()
        )
        ...
        (
            self.cmds_route.add_cmd(
                name="rec-start",
                description="Graba ruta por telemetría",
                args=(("plid", int), ("route_name", str)),
                funct=self._route_start,
            )
            .add_cmd(
                name="rec-stop",
                description="Para y guarda la ruta",
                args=(("plid", int),),
                funct=self._route_stop,
            )
            .add_cmd(
                name="follow",
                description="Sigue una ruta grabada",
                args=(("route_name", str), ("plid", int)),
                funct=self._cmd_route_follow,
                is_mso_required=True,
            )
            .add_cmd(
                name="test",
                description="Carga AI's en las rutas indicadas",
                args="routes",
                funct=self._test_routes,
                is_mso_required=True,
            )
            .submit()
        )
        ...
        (
            self.map_recorder.cmd_manager.add_cmd(
                name="ai_state",
                description="Muestra el estado de un coche ai",
                args=(("ai_plid", int),),
                funct=self._cmd_ai_state,
                is_mso_required=True,
            )
            .add_cmd(
                name="freeroam",
                description="Activa modo explorador en una AI",
                args=(("plid_ai", int),),
                funct=self._cmd_map_freeroam,
                is_mso_required=True,
            )
            .add_cmd(
                name="test",
                description="Spawnea AI's hasta el <num> indicado activandoles el modo freeroam",
                args=(("num", int),),
                funct=self._test_freeroam,
                is_mso_required=True,
            )
            .add_cmd(
                name="ui",
                description="Abrir interfaz gráfica del mapa",
                args=None,
                funct=self._map_ui_open,
                is_mso_required=True,
            )
            .submit()
        )
    ...
    def _show_my_ais_list(self, packet: ISP_MSO):
        ...
        for plid in plids_ais:
            ...
            self.send_ISP_MSL(
                Msg=f"{TextColors.CYAN}PLID: {TextColors.WHITE}{ai_plid} {TextColors.CYAN}| AIName: {TextColors.WHITE}{ai_name}"
            )
    ...
    def _cmd_stop(self, packet: ISP_MSO, plid: str):
        ...
        behavior.reset_speed()
        self.send_ISP_MSL(
            Msg=f"{TextColors.YELLOW}Control desactivado para PLID {plid}"
        )
    ...
    def _cmd_follow(self, packet: ISP_MSO, plid_ai: int, plid_target: int):
        ...
        if plid_ai == plid_target:
            self.send_ISP_MSL(
                Msg=f"{TextColors.RED}Error: El objetivo no puede ser la propia AI."
            )
            return

        if (
            plid_target not in self.user_manager.players
            and plid_target not in self.user_manager.ais
        ):
            self.send_ISP_MSL(
                Msg=f"{TextColors.RED}Error: El PLID {plid_target} no existe."
            )
            return
    ...
    def _cmd_adaptive(
        ...
        self.send_ISP_MSL(
            Msg=f"{TextColors.GREEN}Modo adaptativo configurado para PLID {plid}"
        )
    ...
    def _cmd_spec(self, plid: int):
        ...
        if plid not in self.user_manager.ais:
            self.send_ISP_MSL(
                Msg=f"{TextColors.RED}Error: PLID {plid} no asociado a ninguna AI"
            )
            return
        ai_name = self.user_manager.ais[plid].ai_name
    ...
    def _cmd_route_follow(self, packet: ISP_MSO, route_name: str, plid: int):
        ...
        if route_name not in self.route_manager.loaded_routes:
            self.send_ISP_MSL(
                Msg=f"{TextColors.RED}Error: La ruta '{route_name}' no existe."
            )
            return
        ...
        self.send_ISP_MSL(
            Msg=f"{TextColors.CYAN}Siguiendo ruta: {TextColors.WHITE}{route_name} {TextColors.CYAN}(IA {plid})"
        )
    ...
    def _route_start(self, plid: int, route_name: str):
        ...
        if plid in self.user_manager.ais:
            ...
            return
        self.send_ISP_MSL(
            Msg=f"{TextColors.RED}Error: El PLID {plid} no está en pista."
        )
    ...
    def _cmd_map_freeroam(self, packet: ISP_MSO, plid_ai: int):
        ...
        if not ai:
            self.send_ISP_MSL(
                Msg=f"{TextColors.RED}Error: El PLID {plid_ai} no es una IA."
            )
            return
        ...
        behavior.reset_direction()
        behavior.reset_speed()
        behavior.active_mode = FreeroamMode(
            speed_limit_bias=random.uniform(-0.05, 1.05)
        )
    ...
    def _cmd_ai_state(self, packet: ISP_MSO, ai_plid: int):
        ...
        if not ai:
            self.send_ISP_MSL(
                Msg=f"^1Error: El PLID {ai_plid} no es una IA o no existe."
            )
            return
        ...
        if not behavior:
            self.send_ISP_MSL(
                UCID=packet.UCID,
                Msg=f"^1Error: La IA {ai.ai_name} no tiene inicializado su AIBehavior.",
            )
            return
        ...
        self.send_ISP_MSL(
            Msg=f"^7Velocidad Objetivo: ^2{t_speed:.1f} km/h ^7| Marcha: ^2{getattr(behavior.gear_mode, 'name', behavior.gear_mode)}"
        )
        ...
        if type(mode).__name__ == "FreeroamMode":
            ...
            self.send_ISP_MSL(
                Msg=f"^7Modo: ^6Freeroam ^7| Maniobra actual: ^6{maneuver} ^7| Estado actual: ^6{state}"
            )
            ...
            self.send_ISP_MSL(
                Msg=f"^7Loc: ^2{c_type} ({c_id}) ^7| Nodo actual: ^2{mode.node_index}"
            )
            ...
            self.send_ISP_MSL(
                Msg=f"^7Intermitente: ^3{blinkers}^7| Sentido contrario: ^2{mode.is_driving_opposing}"
            )
            ...
            self.send_ISP_MSL(
                Msg=f"^7Contexto Vía -> Viene de: ^7{p_road} ^7| Vía general: ^7{c_road}"
            )
    ...
    def _test(self, packet: ISP_MSO, routes: str):
        ...
        if not routes_list:
            self.send_ISP_MSL(
                Msg=f"{TextColors.RED}[TEST] Error: Debes especificar al menos una ruta."
            )
            return
        ...
        for route in routes_list:
            if route not in self.route_manager.loaded_routes:
                self.send_ISP_MSL(
                    Msg=f"{TextColors.RED}[TEST] Error: Falta cargar la ruta '{route}'."
                )
                return

        self.send_ISP_MSL(
            Msg=f"{TextColors.YELLOW}[TEST] Iniciando carga masiva de tráfico..."
        )
    ...
    def _test_freeroam(self, packet: ISP_MSO, num: int):
        ...
        if num < 1 or num > 40:
            ...
            self.send_ISP_MSL(
                Msg=f"{TextColors.RED}[ERROR] El límite debe estar entre 1 y 40 jugadores."
            )
            return
        ...
        if getattr(self, "_is_freeroam_loop_running", False):
            self.send_ISP_MSL(
                Msg=f"{TextColors.YELLOW}[TEST] Límite actualizado a {num} jugadores."
            )
            return
