...
class _PhysicsMixin(_MixinBase):
    def _handle_steering(self, ai: AI) -> list[AIV]:
        ...
        if isinstance(behavior.point_request, int):
            ...
            if target_plid in self.user_manager.players:
                ...
                self.logger.warning(
                    f"La IA {ai.ai_name} ha perdido a su objetivo (PLID {target_plid})."
                )
        ...
        # Volante
        lfs_steer = self._calculate_steering(
            behavior=behavior, telemetry=ai.player.telemetry
        )
    ...
    def _calculate_steering(self, behavior: AIBehavior, telemetry: Telemetry) -> int:
        ...
        steer_val = behavior.pid_direction.update(
            target=0.0, current=error, dt=self.interval_mci_s
        )
    ...
    def _handle_pedals_and_gears(self, ai: AI) -> list[AIV]:
        ...
        if isinstance(behavior.speed_request, float):
            ...
            if behavior.point_resolved is not None:
                ...
                speed_at_min = (
                    config.max_speed if behavior.logic_reversed else config.min_speed
                )
                speed_at_max = (
                    config.min_speed if behavior.logic_reversed else config.max_speed
                )
        ...
        # --- CÁLCULO DE PEDALES ---
        lfs_throttle, lfs_brake = self._calculate_pedals(
            behavior, ai.player.telemetry.speed.speed_kmh
        )
    ...
    def _calculate_pedals(
        self, behavior: AIBehavior, current_speed_kmh: float
    ) -> tuple[int, int]:
        """
