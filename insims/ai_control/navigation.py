...
class _NavigationMixin(_MixinBase):
    def _update_route_navigation(self, ai: AI):
        ...
        if getattr(mode, "route_started", False) is False:
            closest_idx = get_closest_node_index(
                my_coords, route.waypoints, is_waypoint=True
            )
            mode.route_wp_index = determine_smart_spawn_index(
                my_coords, closest_idx, route.waypoints, is_waypoint=True
            )
    ...
    def _get_radar_speed_limit(
        ...
        local_wp_density_m = max(
            1.0,
            calc_dist_3d(wp_c.x_m, wp_c.y_m, wp_c.z_m, wp_n.x_m, wp_n.y_m, wp_n.z_m),
        )
        ...
        for other_ai in self.user_manager.ais.values():
            ...
            if (
                not isinstance(other_mode, RouteMode)
                or other_mode.active_route_name != mode.active_route_name
            ):
                continue
        ...
        if (
            gap_filling_mode
            and ais_on_route > 0
            and global_wps_ahead != float("inf")
            and global_wps_behind != float("inf")
        ):
            ...
            if total_gap > 0:
                ...
                if balance < 0.45:
                    target_speed = (
                        base_speed * 0.75
                    )  # Está muy pegado al de delante -> Frena para retrasarse
                elif balance > 0.55:
                    target_speed = (
                        base_speed * 1.20
                    )  # Está muy separado del de delante -> Acelera para cazarlo
        ...
        if closest_dist_ahead_m <= min_dist_m:
            ...
            behavior.ignore_human = (
                True  # [!] Mantener la distancia requiere matemáticas robóticas.
            )
    ...
    def _update_freeroam_navigation(self, ai: AI):
        ...
        if es_parada_intencionada:
            ...
            behavior.stuck_start_time > 0.0
            and (time.time() - behavior.stuck_start_time) >= 3.0
        ):
        ...
        if not mode.freeroam_road_started:
            ctx: LocationContext = self.map_recorder.get_location_context(
                my_coords.x_m,
                my_coords.y_m,
                my_coords.z_m,
                find_zones=False,
                ignore_closed_roads=True,
            )
            ...
            if ctx.link_dist < ctx.road_dist and ctx.link_type == "RoadLink":
                ...
                from_road = (
                    self.map_recorder.roads.get(link_obj.from_road_id)
                    if link_obj
                    else None
                )
                link_from_open = bool(
                    link_obj and from_road and not from_road.is_closed
                )
            ...
            if link_from_open:
                ...
                mode.node_index = closest_idx = get_closest_node_index(
                    my_coords, link_obj.nodes, is_waypoint=False
                )
                ...
                mode.node_index = determine_smart_spawn_index(
                    my_coords, closest_idx, geom.nodes, is_waypoint=False
                )
        ...
        if _in_overtake and mode.overtake_change_lane and mode.overtake_lat_link_id:
            ...
            mode.next_link_id and mode.next_link_type == "LatLink" and not _in_overtake
        ):
        ...
        if _active_lat_id:
            ...
            closest_idx = get_closest_node_index(
                my_coords, lat_link_obj.nodes, is_waypoint=False
            )
            ...
            if not mode.future_indicator:
                _dest_road_id = (
                    lat_link_obj.road_b
                    if lat_link_obj.road_a == mode.current_road_id
                    else lat_link_obj.road_a
                )
                _is_in_fast_lane = (
                    _is_overtake_lat
                    and mode.current_road_id == mode.overtake_fast_lane_id
                )
            ...
            if (
                time_to_link <= behavior.human_safe_gap
                and mode.blinkers_active != mode.future_indicator
            ):
            ...
            _curr_geom = (
                ...
                else self.map_recorder.road_links.get(mode.current_id)
            ...
            if _curr_geom and len(_curr_geom.nodes) >= 2:
                if mode.is_driving_opposing:
                    ...
                    _fwd_x = _curr_geom.nodes[_ni + 1].x_m - _curr_geom.nodes[_ni].x_m
                    _fwd_y = _curr_geom.nodes[_ni + 1].y_m - _curr_geom.nodes[_ni].y_m
                _dot = _fwd_x * (target_node.x_m - my_coords.x_m) + _fwd_y * (
                    target_node.y_m - my_coords.y_m
                )
                _at_side = _dot <= 0
            ...
            if _at_side:
                ...
                mode.current_type = "Road"
                mode.node_index = get_closest_node_index(
                    my_coords, target_road_obj.nodes, is_waypoint=False
                )
        ...
        # --- CASO ROADLINK ---
            ...
            mode.next_link_id and mode.next_link_type == "RoadLink" and not _in_overtake
        ):
            next_link_obj = self.map_recorder.road_links[mode.next_link_id]
            closest_idx = get_closest_node_index(
                my_coords, next_link_obj.nodes, is_waypoint=False
            )
            target_node = next_link_obj.nodes[closest_idx]

            dist_to_link = calc_dist_3d(
                my_coords.x_m,
                my_coords.y_m,
                my_coords.z_m,
                target_node.x_m,
            ...
            if (
                time_to_link <= next_link_obj.time
                and mode.blinkers_active != next_link_obj.indicators
            ):
            ...
            if dist_to_link < TRIGGER_DIST_M:
                ...
                mode.node_index = determine_smart_spawn_index(
                    my_coords, closest_idx, next_link_obj.nodes, is_waypoint=False
                )
                self._plan_next_link(
                    mode, on_link=(next_link_obj, ai.player.telemetry.coordinates)
                )
        ...
        avanzamos = (
            (next_index < mode.node_index)
            if is_opposing
            else (next_index > mode.node_index)
        )
        ...
        if avanzamos:
            ...
            fin_de_geometria = (
                (mode.node_index < 0)
                if is_opposing
                else (mode.node_index >= len(nodes_list))
            )
            ...
            if fin_de_geometria:
                ...
                if is_opposing:
                    self.logger.warning(
                        f"IA {ai.ai_name} se quedó sin asfalto en sentido contrario. Forzando retorno de emergencia."
                    )
                    mode.is_driving_opposing = False
                    mode.overtake_state = "RETURNING"
                    mode.node_index = (
                        0  # Prevenimos crash por OutOfIndex mientras la FSM reincorpora
                    )
                    return
                ...
                if mode.current_type == "RoadLink":
                    ...
                    mode.node_index = determine_smart_spawn_index(
                        my_coords, closest_idx, target_road_obj.nodes, is_waypoint=False
                    )
                    return
        ...
        if getattr(mode, "blinkers_active_now", None) != mode.blinkers_active:
            mode.extra_inputs_to_send.append(
    ...
    def _get_indicator_to_use(
        self,
        my_road_nodes: List[Coordinates],
        other_road_nodes: List[Coordinates],
        node_index,
    ...
    def _get_raw_candidates(
        self, current_road_id: str
    ) -> List[Tuple[str, str, str, List[Coordinates]]]:
        """
    ...
    def _calculate_next_link(
        ...
        for l_id, l_type, target_road_id, link_nodes in candidatos:
            ...
            if not current_road.is_circular:
                if not self._is_link_reachable_ahead(
                    current_road.nodes, current_index, link_nodes, max_dist=8.0
                ):
                    continue
    ...
    def _plan_next_link(
        self, mode: FreeroamMode, on_link: Optional[Tuple[RoadLink, Coordinates]] = None
    ):
        ...
        if on_link:
            ...
            node_index, _ = self._get_closest_node_index(
                coordinates.x_m, coordinates.y_m, nodes
            )
            n_id, n_type = self._calculate_next_link(
                link_obj.to_road_id, link_obj.from_road_id, node_index
            )
        else:
    ...
    def _get_closest_node_index(
        self, px: float, py: float, nodes: List[Coordinates]
    ) -> tuple[int, float]:
