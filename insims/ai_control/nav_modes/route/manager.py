...
class RouteManager(PacketSenderMixin):
    ...
    def start(self, player: "Player", name: str):
        ...
        self.send(
            ISP_MSL(
                Msg=f"^2[REC] Grabación iniciada: ^7{name} siguiendo al plid {player.plid}"
            )
        )
    ...
    def stop(self, plid: int):
        ...
        if not recorder:
            self.send(
                ISP_MSL(Msg=f"No se estaba grabando ninguna ruta con el plid {plid}")
            )
            return
        ...
        if not self._save_to_disk(plid):
            self.send(
                ISP_MSL(
                    Msg=f"Error: No se puedo guardar la ruta {recorder.route_name}. Se sigue grabando."
                )
            )
            return
        ...
        self.send(
            ISP_MSL(Msg=f"^1[REC] Guardada: ^7{recorder.route_name} ({count} pts)")
        )
    ...
    def process(self, plid: int, coordinates: Coordinates, speed: Speed):
        ...
        dynamic_dist_threshold = max(
            recorder.MIN_DYNAMIC_DIST_m, min(recorder.MAX_DYNAMIC_DIST_m, ideal_dist)
        )
        ...
        angle_diff_lfs = calc_deviation_angle(
            point_1.x, point_1.y, point_2.x, point_2.y, safe_coords.x, safe_coords.y
        )
        ...
        if dist_m >= dynamic_dist_threshold:
            ...
            abs(safe_speed.speed_kmh - recorder.waypoints[-1].speed.speed_kmh)
            > recorder.SPEED_DELTA_kmh
        ):
            should_save = True
        elif (
            abs(angle_diff_deg) > recorder.ANGLE_DELTA_degrees
        ):  # Usamos grados contra grados
            should_save = True
        ...
        try:
            with open(RUTAS_FILE, "a") as f:
                ...
                for waypoint in self.recorders[plid].waypoints:
                    f.write(
                        f"    (({waypoint.coordinates.x}, {waypoint.coordinates.y}, {waypoint.coordinates.z}), {waypoint.speed.speed_lfs}),\n"
                    )
                f.write("],\n")
            return True
        except Exception as e:
