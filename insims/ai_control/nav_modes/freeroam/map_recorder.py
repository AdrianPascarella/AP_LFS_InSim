...
class MapRecorder(PacketSenderMixin):
    def __init__(
        self,
        get_coords_fn: Callable[[int], Optional[Coordinates]],
        cmd_prefix: str = "!",
    ):
        ...
        self.current_recording: Optional[
            RoadSegment | RoadLink | LateralLink | IntersectionZone | SpecialRule
        ] = None
        ...
        self.recording_plid: Optional[int] = (
            None  # PLID del jugador cuya telemetría se graba
        )
        self._current_cmd_ucid: Optional[int] = (
            None  # UCID del emisor del comando en curso
        )
    ...
    def update_recording(self, player_coords: Coordinates, speed_kmh: float):
        ...
        dynamic_dist_threshold = max(
            self.MIN_DYNAMIC_DIST_m, min(self.MAX_DYNAMIC_DIST_m, ideal_dist)
        )
        ...
        if len(nodes_list) < 2:
            if dist_m >= dynamic_dist_threshold:
                nodes_list.append(current_node)
                logger.info(
                    f"[Auto-Rec] Nodo #2 guardado (Motivo: Distancia inicial {dist_m:.1f}m)."
                )
                if self._node_flash_callback:
        ...
        # Condición B: Curvatura pronunciada (horquillas, giros bruscos)
            ...
            should_save = True
            debug_reason = (
                f"Ángulo ({abs(angle_diff_deg):.1f}º > {self.ANGLE_DELTA_degrees}º)"
            )
    ...
    def get_location_context(
        ...
        ignore_closed_roads: bool = False,
    ) -> LocationContext:
        ...
        if find_roads and self.roads:
            ...
            if closest_road["id"] is not None:
                ...
                closest_node = self.get_closest_geometry(
                    px,
                    py,
                    pz,
                    enumerate(self.roads[ctx.road_id].nodes),
                    lambda node: node,
                )
        ...
        if find_zones and self.zones:
            ...
            for z_id, zone in self.zones.items():
                ...
                if num_nodos == 1:
                    ...
                    dist_to_shape = calc_dist_point_to_segment_2d(
    ...
    def _register_commands(self):
        ...
        (
            ...
            .add_cmd(
                "set_map",
                "Define el mapa actual o crea uno nuevo",
                (("map_name", str),),
                self._cmd_set_map,
            )
            .add_cmd(
            ...
            .add_cmd(
                "del_map",
                "¡Elimina un mapa entero del disco duro!",
                (("map_name", str),),
                self._cmd_del_map,
            )
            .add_cmd(
                "merge",
                "Fusiona otro fichero de mapa en el mapa activo (no sobreescribe IDs existentes)",
                (("map_name", str),),
                self._cmd_merge_map,
            )
            # ==========================================
            ...
            .add_cmd(
                "rec_road",
                "Graba los nodos de una vía nueva o existente (usa el mismo ID para actualizarla)",
                (("road_id", str),),
                self._cmd_rec_road,
            )
            .add_cmd(
                "rec_roadlink",
                "Graba un enlace entre dos vías (origen y destino) o actualiza su geometría si ya existe",
                (("origen", str), ("destino", str)),
                self._cmd_rec_roadlink,
            )
            .add_cmd(
                "rec_laterallink",
                "Graba un enlace entre dos carriles (izquierdo y derecho) o actualiza su geometría si ya existe",
                (("road_a", str), ("road_b", str)),
                self._cmd_rec_laterallink,
            )
            .add_cmd(
                "rec_zone",
                "Crea o actualiza una zona de intersección grabando su área de nodos y sus reglas de prioridad",
                (("zone_id", str),),
                self._cmd_rec_zone,
            )
            .add_cmd(
                "rec_special_rule",
                "Graba una regla especial (2 nodos: inicio y fin)",
                (("rule_id", str),),
                self._cmd_rec_special_rule,
            )
            # ==========================================
            ...
            .add_cmd(
                "rec_addp",
                "Agrega un nuevo punto a la grabación actual",
                None,
                self._cmd_rec_add_point,
                is_mso_required=True,
            )
            .add_cmd(
                "rec_auto",
                "Agrega puntos automáticamente",
                (("true/false", str),),
                self._cmd_rec_auto,
            )
            .add_cmd(
                "end",
                "Finaliza la grabación actual y guarda los cambios en memoria (usa el mismo ID para actualizar)",
                None,
                self._cmd_rec_end,
            )
            .add_cmd(
                "cancel",
                "Cancela la grabación actual sin guardar los cambios (¡Cuidado, se perderá lo grabado hasta ahora!)",
                None,
                self._cmd_rec_cancel,
            )
            # ==========================================
            ...
            .add_cmd(
                "stats",
                "Muestra el recuento y las listas de objetos en memoria",
                None,
                self._cmd_stats,
            )
            .add_cmd(
                "info",
                "Muestra las propiedades detalladas de un objeto",
                (("id", str),),
                self._cmd_info,
            )
            .add_cmd(
            ...
            .add_cmd(
                "check",
                "Audita el mapa buscando errores de integridad",
                None,
                self._cmd_check_map,
            )
            # ==========================================
            ...
            .add_cmd(
                "set",
                "Edita propiedades de cualquier objeto de forma inteligente",
                (("id", str), ("prop", str), ("val", str)),
                self._cmd_set,
            )
            .add_cmd(
                "del",
                "Elimina un objeto por ID (Borrado en Cascada)",
                (("id", str),),
                self._cmd_del,
            )
            .add_cmd(
                "rec_road_rule",
                "Define la regla de tráfico (RHT o LHT) default",
                (("RHT/LHT", str),),
                self._cmd_rec_road_rule,
            )
            # ==========================================
            # 6. EXTRA
            # ==========================================
            .add_cmd(
                "closed_roads",
                "Muestra un listado de los roads cerrados",
                None,
                self._cmd_is_closed_road,
            )
        )
    ...
    def _cmd_list_maps(self):
        ...
        if not os.path.exists(maps_folder):
            self.send(
                ISP_MSL(Msg=f"{TextColors.YELLOW}La carpeta 'maps' aún no existe.")
            )
            return
        ...
        if not archivos:
            self.send(
                ISP_MSL(Msg=f"{TextColors.YELLOW}No hay mapas guardados todavía.")
            )
            return

        self.send(
            ISP_MSL(
                Msg=f"{TextColors.GREEN}=== {TextColors.WHITE}Mapas Disponibles {TextColors.GREEN}==="
            )
        )
        ...
        if self.active_map_name:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.YELLOW}Mapa activo actual: {TextColors.WHITE}{self.active_map_name}"
                )
            )
    ...
    def _cmd_set_map(self, map_name: str):
        ...
        if self.current_recording is not None:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error: Estás grabando algo. Usa !map cancel primero.",
        ...
        if self.active_map_name == map_name:
            self.send(
                ISP_MSL(Msg=f"{TextColors.YELLOW}El mapa {map_name} ya está activo.")
            )
            return
        ...
        if self._load_map_from_disk(map_name):
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.GREEN}Mapa {TextColors.WHITE}{map_name}{TextColors.GREEN} cargado desde el disco."
                )
            )
        else:
    ...
    def _load_map_from_disk(self, map_name: str) -> bool:
        ...
        try:
            ...
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}El archivo del mapa está corrupto. Revisa la consola.",
                    Sound=SND.INVALIDKEY,
                )
            )
            return False
    ...
    def _cmd_save_map(self):
        ...
        if not self.active_map_name:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error: No has seleccionado ningún mapa. Usa {TextColors.WHITE}!map set_map <nombre>{TextColors.RED} primero.",
                    Sound=SND.INVALIDKEY,
                )
            )
            return
        ...
        if self.current_recording is not None:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error: Termina (!map end) o cancela la grabación actual antes de guardar.",
                    Sound=SND.INVALIDKEY,
                )
            )
            return
        ...
        try:
            ...
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.GREEN}Mapa guardado con éxito: {TextColors.WHITE}{filename}"
                )
            )
            ...
            # =========================================================
            logger.info(
                f"Iniciando hilo en segundo plano para actualizar imagen: {full_path}"
            )
            ...
            img_thread = threading.Thread(
                target=generate_map_image, args=(full_path,), daemon=True
            )
            ...
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error de formato al guardar. Revisa la consola.",
                    Sound=SND.INVALIDKEY,
                )
            )
        except Exception as e:
            ...
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error interno al guardar el mapa. Revisa la consola.",
                    Sound=SND.INVALIDKEY,
                )
            )
    ...
    def _cmd_del_map(self, map_name: str):
        ...
        if os.path.exists(full_path):
            try:
                ...
                self.send(
                    ISP_MSL(
                        Msg=f"{TextColors.GREEN}El mapa {TextColors.WHITE}{map_name}{TextColors.GREEN} ha sido destruido del disco."
                    )
                )
                ...
                if self.active_map_name == map_name:
                    ...
                    self.send(
                        ISP_MSL(
                            Msg=f"{TextColors.YELLOW}El mapa activo ha sido borrado. Memoria RAM reseteada."
                        )
                    )
                ...
                self.send(
                    ISP_MSL(
                        Msg=f"{TextColors.RED}Error de sistema al borrar el archivo. Revisa la consola.",
                        Sound=SND.INVALIDKEY,
                    )
                )
        else:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error: No se encontró el archivo '{map_name}.json'.",
                    Sound=SND.INVALIDKEY,
                )
            )
    ...
    def _cmd_merge_map(self, map_name: str):
        ...
        if not self.active_map_name:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Activa un mapa primero con !map set_map <nombre>.",
                    Sound=SND.INVALIDKEY,
                )
            )
            return
        ...
        if not os.path.exists(full_path):
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}No se encontró '{map_name}.json'.",
                    Sound=SND.INVALIDKEY,
                )
            )
            return
        ...
        try:
            ...
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}El archivo está corrupto. Revisa la consola.",
                    Sound=SND.INVALIDKEY,
                )
            )
            return
        ...
        self.send(
            ISP_MSL(
                Msg=f"{TextColors.GREEN}Merge completado: {TextColors.WHITE}{added}{TextColors.GREEN} objetos añadidos, {TextColors.YELLOW}{skipped}{TextColors.GREEN} omitidos (ID ya existía)."
            )
        )
        if skipped_ids:
            preview = ", ".join(skipped_ids[:5])
            suffix = (
                f"... (+{len(skipped_ids) - 5} más)" if len(skipped_ids) > 5 else ""
            )
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.YELLOW}IDs omitidos: {TextColors.WHITE}{preview}{suffix}"
                )
            )
        logger.info(
            f"Merge de '{map_name}' en '{self.active_map_name}': +{added} añadidos, {skipped} omitidos."
        )
    ...
    def _cmd_rec_road(self, road_id: str):
        ...
        if not self.active_map_name:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error: Selecciona un mapa primero con {TextColors.WHITE}!map set_map <nombre>",
                    Sound=SND.INVALIDKEY,
                )
            )
            return
        ...
        if self.current_recording is not None:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Ya hay una grabación en curso. Escribe !map end o !map cancel primero.",
                    Sound=SND.INVALIDKEY,
                )
            )
            return
        ...
        if self.current_recording["is_new"]:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.GREEN}Grabando NUEVA vía: {TextColors.WHITE}{road_id}{TextColors.GREEN}."
                )
            )
        else:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.YELLOW}Re-grabando vía: {TextColors.WHITE}{road_id}{TextColors.YELLOW}. La original se mantendrá hasta que uses !map end."
                )
            )
        ...
        self.send(
            ISP_MSL(
                Msg=f"{TextColors.CYAN}Usa !map rec_addp para nodos manuales o !map rec_auto true para grabar en movimiento."
            )
        )
    ...
    def _cmd_rec_roadlink(self, origen: str, destino: str):
        ...
        if from_road not in self.roads:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.YELLOW}Advertencia: La vía origen '{TextColors.WHITE}{from_road}{TextColors.YELLOW}' aún no existe en memoria."
                )
            )
        if to_road not in self.roads:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.YELLOW}Advertencia: La vía destino '{TextColors.WHITE}{to_road}{TextColors.YELLOW}' aún no existe en memoria."
                )
            )
        ...
        if is_new:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.GREEN}Grabando NUEVO LINK: {TextColors.WHITE}{link_id}{TextColors.GREEN}."
                )
            )
        else:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.YELLOW}Re-grabando LINK: {TextColors.WHITE}{link_id}{TextColors.YELLOW}. El original se mantendrá hasta usar !map end."
                )
            )

        self.send(
            ISP_MSL(
                Msg=f"{TextColors.CYAN}Conduce la trayectoria y usa !map end al terminar."
            )
        )
    ...
    def _cmd_rec_laterallink(self, road_a: str, road_b: str):
        ...
        if not self.active_map_name:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error: Selecciona un mapa primero con {TextColors.WHITE}!map set_map",
                    Sound=SND.INVALIDKEY,
                )
            )
            return
        ...
        if self.current_recording:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Ya hay una grabación en curso. Escribe !map end o cancel primero.",
                    Sound=SND.INVALIDKEY,
                )
            )
            return
        ...
        if a_road not in self.roads:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.YELLOW}Aviso: El carril izquierdo '{TextColors.WHITE}{a_road}{TextColors.YELLOW}' no existe aún."
                )
            )
        if b_road not in self.roads:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.YELLOW}Aviso: El carril derecho '{TextColors.WHITE}{b_road}{TextColors.YELLOW}' no existe aún."
                )
            )
        ...
        if is_new:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.GREEN}Grabando NUEVO ENLACE LATERAL: {TextColors.WHITE}{link_id}"
                )
            )
        else:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.YELLOW}Re-grabando ENLACE LATERAL: {TextColors.WHITE}{link_id}"
                )
            )

        self.send(
            ISP_MSL(
                Msg=f"{TextColors.CYAN}Traza el cambio de carril y usa !map end al terminar."
            )
        )
    ...
    def _cmd_rec_zone(self, zone_id: str):
        ...
        if is_new:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.GREEN}Grabando NUEVA ZONA de conflicto: {TextColors.WHITE}{zone_id}"
                )
            )
        else:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.YELLOW}Re-grabando ZONA de conflicto: {TextColors.WHITE}{zone_id}"
                )
            )
        ...
        self.send(
            ISP_MSL(
                Msg=f"{TextColors.CYAN}Usa !map rec_addp para marcar y !map end al terminar."
            )
        )
    ...
    def _cmd_rec_special_rule(self, rule_id: str):
        ...
        if is_new:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.GREEN}Grabando NUEVA REGLA ESPECIAL: {TextColors.WHITE}{rule_id}"
                )
            )
        else:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.YELLOW}Re-grabando REGLA ESPECIAL: {TextColors.WHITE}{rule_id}"
                )
            )

        self.send(
            ISP_MSL(Msg=f"{TextColors.CYAN}Captura EXACTAMENTE 2 nodos: inicio y fin.")
        )
        self.send(
            ISP_MSL(
                Msg=f"{TextColors.CYAN}Usa !map rec_addp para marcar y !map end al terminar."
    ...
    def _cmd_rec_add_point(self, packet: ISP_MSO):
        ...
        if not self.current_recording:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error: No hay ninguna grabación en curso. Usa !map rec_road, rec_zone, etc.",
                    Sound=SND.INVALIDKEY,
                )
            )
            return
        ...
        if not coords:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error: No se pudo obtener tu telemetría (¿Estás en pista?).",
                    Sound=SND.INVALIDKEY,
                )
            )
            return
        ...
        if self._node_flash_callback:
            ...
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.GREEN}Punto de {tipo} añadido {TextColors.WHITE}(Nodo/Vértice {count}){TextColors.GREEN}."
                )
            )
    ...
    def _cmd_rec_auto(self, state_str: str):
        ...
        if state_lower in ["true", "on", "1", "si", "yes", "t"]:
            ...
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error: Usa {TextColors.WHITE}!map rec_auto true{TextColors.RED} o {TextColors.WHITE}false{TextColors.RED}.",
                    Sound=SND.INVALIDKEY,
                )
            )
            return
        ...
        if new_state and not self.current_recording:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.YELLOW}Aviso: Autograbado listo, pero recuerda iniciar una grabación primero (!map rec_road, etc.)."
                )
            )
        ...
        if new_state:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.GREEN}Autograbado {TextColors.WHITE}ACTIVADO{TextColors.GREEN}. Conduce para trazar la ruta automáticamente."
                )
            )
        else:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.YELLOW}Autograbado {TextColors.WHITE}DESACTIVADO{TextColors.YELLOW}. Usa !map rec_addp para puntos manuales."
                )
            )
    ...
    def _cmd_rec_end(self):
        ...
        if not self.current_recording:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error: No hay ninguna grabación activa.",
                    Sound=SND.INVALIDKEY,
                )
            )
            return
        ...
        if not nodes:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.YELLOW}Aviso: No se ha grabado ningún nodo. Operación cancelada."
                )
            )
            self.current_recording = None
        ...
        try:
            ...
            # ==========================================
                ...
                if len(nodes) != 2:
                    self.send(
                        ISP_MSL(
                            Msg=f"{TextColors.RED}Error: Una SpecialRule requiere exactamente 2 nodos (inicio y fin). Grabados: {len(nodes)}.",
                            Sound=SND.INVALIDKEY,
                        )
                    )
                    return
                if not is_new and obj_id in self.special_rules:
                    ...
                    self.special_rules[obj_id] = SpecialRule(
                        rule_id=obj_id, nodes=nodes
                    )
                ...
                self.send(
                    ISP_MSL(
                        Msg=f"{TextColors.RED}Error crítico: Tipo de geometría desconocido.",
                        Sound=SND.INVALIDKEY,
                    )
                )
                return
            ...
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.GREEN}{msg_tipo} '{TextColors.WHITE}{obj_id}{TextColors.GREEN}' {accion} con {len(nodes)} nodos.{mensaje_extra}"
                )
            )
            ...
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error interno al guardar. Revisa la consola.",
                    Sound=SND.INVALIDKEY,
                )
            )
    ...
    def _cmd_rec_cancel(self):
        ...
        if not self.current_recording:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error: No hay ninguna grabación activa en este momento.",
                    Sound=SND.INVALIDKEY,
                )
            )
            return
        ...
        self.send(
            ISP_MSL(
                Msg=f"{TextColors.YELLOW}Grabación de '{geom_type}' cancelada. Se han descartado los nodos."
            )
        )
    ...
    def _cmd_stats(self):
        ...
        if not self.active_map_name:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error: No hay ningún mapa activo ({TextColors.WHITE}!map set_map{TextColors.RED})."
                )
            )
            return
        ...
        self.send(
            ISP_MSL(
                Msg=f"{TextColors.GREEN}--- Info de: {TextColors.WHITE}{self.active_map_name} {TextColors.GREEN}---"
            )
        )
        ...
        def enviar_resumen_id(titulo: str, color_titulo: str, ids: list):
            if not ids:
                self.send(
                    ISP_MSL(Msg=f"{color_titulo}{titulo}: {TextColors.WHITE}Ninguno")
                )
                return
            ...
            if len(ids) > MAX_IDS_DISPLAY:
                texto_unido += (
                    f" {TextColors.YELLOW}(+{len(ids) - MAX_IDS_DISPLAY} más...)"
                )

            # Sistema de troceado (Tu lógica de chunks de 100 caracteres)
            MAX_LEN = 100
            chunks = [
                texto_unido[i : i + MAX_LEN]
                for i in range(0, len(texto_unido), MAX_LEN)
            ]
            ...
            self.send(
                ISP_MSL(
                    Msg=f"{color_titulo}{titulo} {conteo}{TextColors.WHITE}{chunks[0]}"
                )
            )
        ...
        enviar_resumen_id(
            "LatLinks", TextColors.YELLOW, list(self.lateral_links.keys())
        )
        enviar_resumen_id(
            "SpecialRules", TextColors.YELLOW, list(self.special_rules.keys())
        )
        ...
        if self.current_recording:
            ...
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}>> GRABANDO: {TextColors.WHITE}{tipo} ({nodos} nodos)"
                )
            )
    ...
    def _cmd_info(self, obj_id: str):
        ...
        if not self.active_map_name:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error: Selecciona un mapa primero (!map set_map <nombre>)."
                )
            )
            return
        ...
        if obj_id in self.roads:
            ...
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error: ID '{obj_id}' no encontrado en Vías, Zonas, Links o SpecialRules."
                )
            )
            return
        ...
        for prop_name, val in atributos_a_mostrar:
            ...
            # Caso D: Enums (TrafficRule, IndicatorType...)
                ...
                display_val = (
                    val.name
                )  # Muestra "RHT" o "OFF" en vez del objeto en memoria
                editable_tag = (
                    f"{TextColors.GREEN}[EDIT: !map set {obj_id} {prop_name} <val>]"
                )
            ...
            # Caso E: Valores básicos editables (Booleanos, strings, ints y floats)
                ...
                if isinstance(val, bool):
                    display_val = (
                        f"{TextColors.GREEN}Sí" if val else f"{TextColors.RED}No"
                    )
            ...
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.WHITE}{prop_name}: {TextColors.YELLOW}{display_val} {editable_tag}"
                )
            )

    @staticmethod
    def get_closest_geometry(
        px: float, py: float, pz: float, items_iterator: iter, extract_geom_fn: callable
    ) -> dict:
        """
        Busca la geometría más cercana (punto o segmentos) a unas coordenadas 3D.
        """
        best_match = {"id": None, "dist": float("inf"), "item": None, "ref_node": None}
        ...
        for item_id, item in items_iterator:
            ...
            if isinstance(geom, (list, tuple)):
                ...
                # Si tiene 2 o más, se trata como segmentos de línea
                    ...
                    for i in range(len(geom) - 1):
                        A, B = geom[i], geom[i + 1]
                        dist = calc_dist_point_to_segment_3d(
                            px, py, pz, A.x_m, A.y_m, A.z_m, B.x_m, B.y_m, B.z_m
                        )
                        if dist < best_match["dist"]:
                            best_match.update(
                                {
            ...
            # Caso B: Es un nodo simple (Punto a Punto / Centro)
                ...
                if dist < best_match["dist"]:
                    best_match.update(
                        {"id": item_id, "dist": dist, "item": item, "ref_node": geom}
                    )
    ...
    def _cmd_whereami(self, packet: ISP_MSO, target_type: str):
        ...
        if not self.active_map_name:
            self.send(
                ISP_MSL(Msg=f"{TextColors.RED}Error: No hay ningún mapa cargado.")
            )
            return
        ...
        if search_by not in ["road", "link", "zone", "all"]:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error: Usa '!map whereami [road/link/zone/all]'"
                )
            )
            return
        ...
        self.send(
            ISP_MSL(
                Msg=f"{TextColors.CYAN}Tus Coordenadas: {TextColors.WHITE}X:{px:.1f} Y:{py:.1f} Z:{pz:.1f}"
            )
        )
        ...
        if search_by in ["road", "all"]:
            if ctx.road_id and ctx.road_dist <= SEARCH_RADIUS_M:
                self.send(
                    ISP_MSL(
                        Msg=f"{TextColors.GREEN}Vía: {TextColors.WHITE}{ctx.road_id} {TextColors.GREEN}| Nodo: {TextColors.WHITE}{ctx.road_node_idx} {TextColors.GREEN}| Dist: {ctx.road_dist:.1f}m"
                    )
                )
            elif ctx.road_id:
                self.send(
                    ISP_MSL(
                        Msg=f"{TextColors.YELLOW}Fuera de vía. Más cercana: {TextColors.WHITE}{ctx.road_id} {TextColors.YELLOW}a {ctx.road_dist:.1f}m."
                    )
                )
            else:
        ...
        if search_by in ["link", "all"]:
            if ctx.link_id:
                self.send(
                    ISP_MSL(
                        Msg=f"{TextColors.GREEN}{ctx.link_type} '{ctx.link_id}' cercano a {ctx.link_dist:.1f}m"
                    )
                )
            else:
                self.send(
                    ISP_MSL(Msg=f"{TextColors.YELLOW}No se encontró ningún link cerca.")
                )
        ...
        if search_by in ["zone", "all"]:
            if ctx.zone_id:
                if ctx.zone_dist <= ctx.zone_radius:
                    ...
                    self.send(
                        ISP_MSL(
                            Msg=f"{TextColors.YELLOW}FUERA. Zona más cercana: {TextColors.WHITE}{ctx.zone_id} {TextColors.YELLOW}a {ctx.zone_dist:.1f}m del límite."
                        )
                    )
            else:
    ...
    def collect_check_results(self) -> tuple[list, list]:
        ...
        for link_id, link in self.road_links.items():
            ...
            if link.from_road_id not in self.roads:
                errores.append(
                    f"[RoadLink {link_id}] Origen '{link.from_road_id}' no existe."
                )
            if link.to_road_id not in self.roads:
                errores.append(
                    f"[RoadLink {link_id}] Destino '{link.to_road_id}' no existe."
                )
            if link.by_road_id:
                if link.by_road_id not in self.roads:
                    advertencias.append(
                        f"[RoadLink {link_id}] 'by_road_id' apunta a via fantasma '{link.by_road_id}'."
                    )
                elif link.from_road_id in self.roads:
                    es_vecino = any(
                        (
                            ll.road_a == link.from_road_id
                            and ll.road_b == link.by_road_id
                        )
                        or (
                            ll.road_b == link.from_road_id
                            and ll.road_a == link.by_road_id
                        )
                        for ll in self.lateral_links.values()
                    )
                    if not es_vecino:
                        advertencias.append(
                            f"[RoadLink {link_id}] '{link.by_road_id}' no esta enlazado lateralmente con '{link.from_road_id}'."
                        )
        ...
        for ll_id, llink in self.lateral_links.items():
            ...
            if llink.road_a not in self.roads:
                errores.append(
                    f"[LatLink {ll_id}] 'road_a' ({llink.road_a}) no existe."
                )
        ...
        for r_id, road in self.roads.items():
            ...
            has_entry_link = any(
                link.to_road_id == r_id for link in self.road_links.values()
            )
            has_lateral_entry = any(
                (lat.road_b == r_id and lat.allow_a_to_b)
                or (lat.road_a == r_id and lat.allow_b_to_a)
                for lat in self.lateral_links.values()
            )
            if not has_entry_link and not has_lateral_entry:
                advertencias.append(
                    f"[Via {r_id}] Inaccesible (No hay RoadLinks ni LatLinks que apunten hacia ella)."
                )
            if not road.is_circular:
                has_exit_link = any(
                    l.from_road_id == r_id for l in self.road_links.values()
                )
                has_lateral_exit = any(
                    (lat.road_a == r_id and lat.allow_a_to_b)
                    or (lat.road_b == r_id and lat.allow_b_to_a)
                    for lat in self.lateral_links.values()
                )
                if not has_exit_link and not has_lateral_exit:
                    advertencias.append(
                        f"[Via {r_id}] Callejon sin salida (Faltan enlaces directos o LatLinks para salir)."
                    )
    ...
    def _cmd_check_map(self):
        ...
        self.send(
            ISP_MSL(
                Msg=f"{TextColors.CYAN}--- Chequeando integridad de: {self.active_map_name} ---"
            )
        )
        ...
        if tot_err == 0 and tot_adv == 0:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.GREEN}[OK] Mapa perfecto! La topologia es 100% solida."
                )
            )
            return
        ...
        self.send(
            ISP_MSL(
                Msg=f"{msg_color}Chequeo finalizado: {TextColors.WHITE}{tot_err} errores {msg_color}y {TextColors.WHITE}{tot_adv} avisos."
            )
        )
        ...
        if tot_err == 0 or tot_err <= 4:
            for adv in advertencias[:4]:
                self.send(
                    ISP_MSL(
                        Msg=f"{TextColors.YELLOW}AVISO: {TextColors.WHITE}{adv[:80]}"
                    )
                )
            if tot_adv > 4:
                self.send(
                    ISP_MSL(Msg=f"{TextColors.YELLOW}... y {tot_adv - 4} avisos mas.")
                )
    ...
    def _get_collection_and_type(self, obj_id: str):
        ...
        if obj_id in self.road_links:
            return self.road_links, "RoadLink"
        if obj_id in self.lateral_links:
            return self.lateral_links, "LatLink"
        if obj_id in self.zones:
            return self.zones, "Zone"
        if obj_id in self.special_rules:
            return self.special_rules, "SpecialRule"
        return None, None
    ...
    def _cmd_set(self, obj_id: str, prop: str, val_str: str):
        ...
        if not collection:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error: ID '{obj_id}' no encontrado en ninguna categoria."
                )
            )
            return
        ...
        prop = (
            prop.lower()
        )  # Asegurar lowercase para las propiedades de las Dataclasses
        ...
        if prop in inmutables:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}La propiedad '{prop}' es inmutable tras la creacion."
                )
            )
            return
        ...
        if prop == "nodes":
            comando = (
                "!map rec_road" if obj_type == "Road" else "!map rec_link"
            )  # Simplificado
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Los nodos no se editan a mano. Usa: {TextColors.WHITE}{comando} {obj_id}"
                )
            )
            return
        ...
        if not hasattr(obj, prop) and not (
            ...
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}La propiedad '{prop}' no existe en este objeto ({obj_type})."
                )
            )
            return
        ...
        try:
            ...
            if prop == "indicators":
                ...
                if modo in ["off", "apagado", "none"]:
                    ...
                    self.send(
                        ISP_MSL(
                            Msg=f"{TextColors.RED}Error: Valor invalido. Usa: off | left | right | hazard"
                        )
                    )
                    return
                ...
                self.send(
                    ISP_MSL(
                        Msg=f"{TextColors.GREEN}Intermitente configurado a '{modo.upper()}' en {obj_id}."
                    )
                )
                return
            ...
            if prop == "traffic_rule":
                ...
                if modo in ["rht", "derecha", "0"]:
                    ...
                    self.send(
                        ISP_MSL(
                            Msg=f"{TextColors.RED}Error: Usa 'rht' (der), 'lht' (izq) o 'none'."
                        )
                    )
                    return
                ...
                self.send(
                    ISP_MSL(
                        Msg=f"{TextColors.GREEN}Norma de trafico configurada en {obj_id}."
                    )
                )
                return
            ...
            if obj_type == "SpecialRule" and prop in ["speed_limit", "no_lane_change"]:
                if prop == "speed_limit":
                    ...
                    self.send(
                        ISP_MSL(
                            Msg=f"{TextColors.GREEN}speed_limit = {obj.rules['speed_limit']} km/h en {obj_id}."
                        )
                    )
                else:
                    obj.rules["no_lane_change"] = val_str.lower() in [
                        "true",
                        "1",
                        "si",
                        "yes",
                    ...
                    self.send(
                        ISP_MSL(
                            Msg=f"{TextColors.GREEN}no_lane_change = {obj.rules['no_lane_change']} en {obj_id}."
                        )
                    )
                return
            ...
            if prop == "rules" and obj_type == "SpecialRule":
                ...
                if operacion == "clear":
                    ...
                    self.send(
                        ISP_MSL(
                            Msg=f"{TextColors.GREEN}Todas las reglas borradas en {obj_id}."
                        )
                    )
                    return
                ...
                if len(partes) != 2:
                    self.send(
                        ISP_MSL(
                            Msg=f"{TextColors.RED}Uso: clear | speed_limit;<valor> | no_lane_change;<true/false>"
                        )
                    )
                    return
                ...
                if clave == "speed_limit":
                    ...
                    obj.rules["no_lane_change"] = valor_str.lower() in [
                        "true",
                        "1",
                        "si",
                        "yes",
            ...
            if val_str.lower() == "none":
                ...
                if prop not in campos_permitidos_none:
                    self.send(
                        ISP_MSL(
                            Msg=f"{TextColors.RED}Error: La propiedad '{prop}' es obligatoria y no puede ser 'None'."
                        )
                    )
                    return
                new_val = None

            elif isinstance(current_val, bool) or prop in [
                "is_circular",
                "is_closed",
                "allow_a_to_b",
                "allow_b_to_a",
            ...
            elif isinstance(current_val, (int, float)) and not isinstance(
                current_val, bool
            ):
                new_val = (
                    float(val_str) if isinstance(current_val, float) else int(val_str)
                )
            ...
            self.send(
                ISP_MSL(Msg=f"{TextColors.GREEN}Cambio aplicado: {prop} = {new_val}")
            )
            ...
            if new_val is not None and isinstance(new_val, str):
                if prop in ["by_road_id"] and new_val not in self.roads:
                    self.send(
                        ISP_MSL(
                            Msg=f"{TextColors.YELLOW}Aviso: La via '{new_val}' aun no existe en el mapa."
                        )
                    )
            ...
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.RED}Error: El valor '{val_str}' no coincide con el tipo numerico esperado."
                )
            )
        except Exception as e:
    ...
    def _cmd_del(self, obj_id: str):
        ...
        if not collection:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.YELLOW}[AVISO] No se encontro objeto con el ID '{obj_id}'."
                )
            )
            return
        ...
        if obj_type == "Road":
            ...
            links_to_remove = [
                l_id
                for l_id, link in self.road_links.items()
                if link.from_road_id == obj_id
                or link.to_road_id == obj_id
                or link.by_road_id == obj_id
            ]
            ...
            if r_links_del > 0 or l_links_del > 0 or zonas_mod > 0:
                self.send(
                    ISP_MSL(
                        Msg=f"{TextColors.YELLOW}Cascada: Borrados {r_links_del} R-Links y {l_links_del} L-Links. Limpiadas {zonas_mod} Zonas."
                    )
                )
        ...
        # ==========================================
            ...
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.GREEN}{obj_type} '{obj_id}' eliminado correctamente."
                )
            )
    ...
    def _cmd_rec_road_rule(self, rule_str: str):
        ...
        self.send(
            ISP_MSL(
                Msg=f"{TextColors.GREEN}Nuevas vias se grabaran por defecto por la {TextColors.WHITE}{texto}."
            )
        )
    ...
    def _cmd_is_closed_road(self):
        ...
        if not hasattr(self, "roads") or not self.roads:
            self.send(
                ISP_MSL(Msg=f"{TextColors.RED}Error: No hay vias cargadas en el mapa.")
            )
            return
        ...
        if not rutas_cerradas:
            self.send(
                ISP_MSL(
                    Msg=f"{TextColors.GREEN}INFO: {TextColors.WHITE}Todas las vias estan abiertas. ¡Via libre!"
                )
            )
            return
